...
export const cvStorageService = {
  ...
  async saveCVData(params: {
    ...
    try {
      ...
      console.log('💾 [CV-STORAGE] Saving CV data to Supabase:', {
      ...
      if (error) {
        console.error('❌ [CV-STORAGE] Save error:', error);
        return { success: false, error: error.message };
      }

      if (!data) {
        console.error('❌ [CV-STORAGE] No data returned from upsert');
        return { success: false, error: 'No data returned from database' };
      }

      const cvId = data.id;
      console.log('✅ [CV-STORAGE] CV data saved successfully:', { cvId, status });
      return { success: true, cvId };
    } catch (err: any) {
      console.error('❌ [CV-STORAGE] Save exception:', err);
      return { success: false, error: err.message };
    }
  },
  ...
  async loadCVData(id: string): Promise<{ success: boolean; data?: StoredCV; error?: string }> {
    try {
      ...
      if (error) {
        console.error('❌ [CV-STORAGE] Load error:', error);
        return { success: false, error: error.message };
      }
      ...
      console.error('❌ [CV-STORAGE] Load exception:', err);
      return { success: false, error: err.message };
    }
  },
  ...
  async getUserCVs(userId: string): Promise<{ success: boolean; data?: StoredCV[]; error?: string }> {
    try {
      ...
      if (error) {
        console.error('❌ [CV-STORAGE] Get user CVs error:', error);
        return { success: false, error: error.message };
      }

      return { success: true, data: (data || []) as StoredCV[] };
    } catch (err: any) {
      console.error('❌ [CV-STORAGE] Get user CVs exception:', err);
      return { success: false, error: err.message };
    }
  },
};
