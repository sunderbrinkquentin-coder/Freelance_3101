...
export class DatabaseService {
  ...
  /**
   * Lädt die Datei in den Storage-Bucket `cv_uploads`,
   * legt einen Datensatz in `cv_uploads` an
   * und triggert optional den Make-Webhook.
   *
   * Rückgabe:
   *  - uploadedCv: Datensatz aus `cv_uploads`
   *  - cvUploadId: ID aus `cv_uploads` (für Analyse-Status & Frontend)
   */
  async uploadCV(file: File): Promise<{ uploadedCv: UploadedCV; cvUploadId: string }> {
    const profile = await this.getOrCreateProfile();
    const sessionId = sessionManager.getSessionId();
    ...
    // 2) Signed URL holen (1 Stunde gültig)
    ...
    if (signedUrlError || !publicUrl) {
      throw new Error('Failed to get signed URL for uploaded CV');
    }

    // 3) Eintrag in cv_uploads
    const cvData: UploadedCVInsert = {
      profile_id: profile.id,
      session_id: sessionId,
    ...
    const { data: uploadedCv, error: insertCvError } = await supabase
      .from('stored_cvs')
      .insert(cvData)
      .select()
      .single();

    if (insertCvError || !uploadedCv) {
      throw new Error(`Failed to create CV record: ${insertCvError?.message}`);
    }

    // 4) Use the same ID from cv_uploads
    const cvUploadId = uploadedCv.id as string;

    // 5) Make-Webhook triggern (falls gesetzt)
    ...
    if (webhookUrl) {
      try {
        await fetch(webhookUrl, {
          ...
          body: JSON.stringify({
            upload_id: cvUploadId,
            file_url: publicUrl,
            uploaded_cv_id: uploadedCv.id,
            profile_id: profile.id,
          }),
        });
      } catch (e) {
      ...
      console.warn('VITE_MAKE_WEBHOOK_URL is not set – Make will not be triggered');
    }

    return { uploadedCv, cvUploadId };
  }

  async getUploadedCVs(): Promise<UploadedCV[]> {
    const profile = await this.getOrCreateProfile();
    ...
    if (error) {
      throw new Error(`Failed to fetch uploaded CVs: ${error.message}`);
    }
